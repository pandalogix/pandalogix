...
namespace Engine.CoreTest
{
  ...
  {
    [Fact]
    public async System.Threading.Tasks.Task SubstractNodeConstantTestAsync()
    {
      ...
      var add = new Engine.Contracts.NodeBaseContract()
      {
        ...
        MetaData = new NodeMetaData()
        {
          NodeData = new Engine.NodeMetaDataAttribute() { NodeClass = typeof(SubstractNode) },
          FieldsMetaData = new List<FieldMetaDataAttribute>()
          {
            new FieldMetaDataAttribute()
            {
      ...
      Assert.Equal(9.0, pad.Context.Result);
    }
  }
}
