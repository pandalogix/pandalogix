...
{
    [NodeMetaData(NodeClass = typeof(AddNode), Category = "Math", Name = nameof(AddNode))]
  public class AddNode : DualInputBaseNode
  {
    protected override async Task InternalExcute(IContext context)
    {
      ...
      else if (leftType.IsDateTimeType() && rightType.IsDateTimeType())
      {
        ...
        {
          _context.Result = Convert.ToDateTime(left).Add((TimeSpan)right);
        }
        if (rightType == typeof(DateTime) && leftType == typeof(TimeSpan))
        {
          _context.Result = Convert.ToDateTime(right).Add((TimeSpan)left);
        }
      }
      else
      {
